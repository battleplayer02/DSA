...
public class K_Subsets_with_equal_sum {
    ...
    public static void solution(int[] arr, int vidx, int k, int[] subsetSum, int ssssf,
        ...
        for (int i = 0; i < ans.size(); i++) {
            if (ans.get(i).size() > 0) {
                ...
                solution(arr, 1 + vidx, k, subsetSum, ssssf, ans);
                subsetSum[i] -= arr[vidx];
                ans.get(i).remove(ans.get(i).size() - 1);
            } else {
                ans.get(i).add(arr[vidx]);
                subsetSum[i] += arr[vidx];
                solution(arr, 1 + vidx, k, subsetSum, ssssf + 1, ans);
                subsetSum[i] -= arr[vidx];
                ans.get(i).remove(ans.get(i).size() - 1);
                break;
            }
